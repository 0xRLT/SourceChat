...
static ImVec2           InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL, ImVec2* out_offset = NULL, const bool stop_on_new_line = false, const float wrap_width = 0.0f, const bool keep_trailing_blanks = false);
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        ImVec2 size = CalcItemSize(ImVec2(0,0), text_size.x + style.FramePadding.x * 2.0f, text_size.y + style.FramePadding.y * 2.0f);

        ImRect bb(text_pos, text_pos + size);

        // Render (we don't hide text after ## in this end-user function)
        RenderTextClipped((bb.Min - ImVec2(0.f,1.f)) + style.TextUnformattedAlign, bb.Max, text, NULL, &text_size, style.TextUnformattedAlign, &bb);
    }
    else
    {
        // Long text!
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool disabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, bool disabled)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, disabled);
}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, bool input)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data, input);
}

bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, bool input)
{
    return InputTextEx(label, NULL, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data, input);
}
...
static ImVec2 FindCharPosition(const ImWchar* text_begin, const ImWchar* char_position, const ImWchar* text_end, const float wrap_width)
{
    ImGuiContext& g = *GImGui;
    ImFont* font = g.Font;
    const float line_height = g.FontSize + ImGui::TextLineSpacing;
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, const bool stop_on_new_line, const float word_wrap_width, const bool keep_trailing_blanks)
{
    ImGuiContext& g = *GImGui;
    ImFont* font = g.Font;
    const float line_height = g.FontSize + ImGui::TextLineSpacing;
...
static void    STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj, int line_start_idx)
{
    ...
    const ImVec2 size = InputTextCalcTextSizeW(text + line_start_idx, text + obj->CurLenW, &text_remaining, NULL, true, obj->WordWrapWidth);
    r->x0 = 0.0f;
    r->x1 = size.x;
...
float ImGui::CalcMultilineWordWrapTextHeight(const char *buffer, int size, float width)
{
    ...
    const float fontSize = g.FontSize + ImGui::TextLineSpacing;

    const ImVec2 frame_size = CalcItemSize(ImVec2(width, 0.f), CalcItemWidth(), fontSize * 8.0f + style.FramePadding.y * 2.0f);
    const float word_wrap_width = frame_size.x - style.FramePadding.x * 2.0f;

    TextW.resize(size + 1);
    ImTextStrFromUtf8(TextW.Data, TextW.Size, buffer, NULL, NULL);

    const ImWchar *text_begin = TextW.Data;
    const ImWchar *text_end = TextW.Data + ImStrlenW(TextW.Data);

    ImVec2 text_size = InputTextCalcTextSizeW(text_begin, text_end, NULL, NULL, false, word_wrap_width);

    return text_size.y;
}

void ImGui::ClearInputText(const char *label)
{
    ImGuiWindow *window = GetCurrentWindow();
    if ( window->SkipItems )
        return;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, bool input)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const bool is_resizable = (flags & ImGuiInputTextFlags_CallbackResize) != 0;
    const bool vertical_scroll = (flags & ImGuiInputTextFlags_NoVerticalScroll) != 0;
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? fontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + frame_size - ImGui::FrameBackgroundOffset );
    const ImRect total_bb(frame_bb.Min, frame_bb.Min + total_size);

    const bool frame_is_hovered = (((io.MousePos.x >= frame_bb.Min.x) && (io.MousePos.x <= frame_bb.Max.x)) && ((io.MousePos.y >= frame_bb.Min.y) && (io.MousePos.y <= frame_bb.Max.y)));
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove | (vertical_scroll ? ImGuiInputTextFlags_NoVerticalScroll : NULL));
        PopStyleVar(3);
        if (!child_visible)
        {
            EndChild();
    ...
    if (g.ActiveId == id)
    {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y) : (fontSize * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const int row_count_per_page = ImMax((int)((inner_size.y - style.FramePadding.y) / fontSize), 1);
        ...
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - fontSize, 0.0f)); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + fontSize, GetScrollMaxY())); else state->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline)      { state->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * fontSize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline)    { state->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * fontSize; }
    ...
    ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ...
    if ( !is_multiline )
    {
        if ( clip_rect.y < frame_bb.Min.y )
            clip_rect.y = frame_bb.Min.y;

        if ( clip_rect.w > frame_bb.Max.y )
            clip_rect.w = frame_bb.Max.y;
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                if (cursor_offset.y - fontSize < scroll_y)
                    scroll_y = ImMax(0.0f, cursor_offset.y - fontSize);
        ...
        bool m_rightclick = io.MouseClicked[1];
        ...
        if (render_selection)
        {
            ...
            {
                if (rect_pos.y > clip_rect.w + fontSize)
                    break;
                ...
                else
                {
                    ...
                    ImVec2 rect_size = InputTextCalcTextSizeW(p, text_selected_end, &p, NULL, true, selection_word_wrap, false);

                    if (rect_size.x <= 0.0f) rect_size.x = IM_FLOOR(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - fontSize), rect_pos + ImVec2(rect_size.x, bg_offy_dn) - ImVec2(0.0f, ImGui::TextLineSpacing));
            ...
            // Check for context menu inputs

            if (m_rightclick && frame_is_hovered)
            {
                io.ContextRightClicked = true;
                io.ContextTextHovered = true;
                io.ContextIsInput = input;
            }
        }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            if ( render_selection )
            {
                const char *text_selected_begin, *text_selected_end;

                if ( buf_display_from_state ) // buffer to render
                {
                    text_selected_begin = state->TextA.Data + ImMin( state->Stb.select_start, state->Stb.select_end );
                    text_selected_end = state->TextA.Data + ImMax( state->Stb.select_start, state->Stb.select_end );
                }
                else
                {
                    text_selected_begin = buf + ImMin( state->Stb.select_start, state->Stb.select_end );
                    text_selected_end = buf + ImMax( state->Stb.select_start, state->Stb.select_end );
                }
        ...
        if ( render_cursor && input && !ImGui::DisableCursorBlinking )
        {
            state->CursorAnim += io.DeltaTime;
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
            ImVec2 cursor_screen_pos = ImFloor(draw_pos + cursor_offset - draw_scroll);
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - fontSize + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f - ImGui::TextLineSpacing);
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), ImGui::CursorColor);
        }
    }
    else
    {
        ...
        if (is_multiline)
            text_size = ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * fontSize); // We don't need width
    ...
    if (io.ContextRightClicked && io.ContextTextHovered)
    {
        if (!io.ContextSetWindowSize)
        {
            ImGui::SetNextWindowPos(io.MousePos);
            ImGui::SetNextWindowSize(io.ContextIsInput ? ImVec2(100, 82) : ImVec2(100, 35));

            io.ContextSetWindowSize = true;
        }

        PushStyleColor(ImGuiCol_Text, IM_COL32(255, 255, 255, 255));
        PushStyleColor(ImGuiCol_HeaderHovered, IM_COL32(255, 156, 0, 255));
        PushStyleColor(ImGuiCol_HeaderActive, IM_COL32(255, 176, 0, 255));

        if (io.ContextIsInput == input)
        {
            if (io.ContextIsInput)
            {
                ImGui::Begin("##context-menu", 0, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing);
                {
                    if (Selectable("Copy", false))
                    {
                        const int ib = state->HasSelection() ? ImMin(state->Stb.select_start, state->Stb.select_end) : 0;
                        const int ie = state->HasSelection() ? ImMax(state->Stb.select_start, state->Stb.select_end) : state->CurLenW;
                        const int clipboard_data_len = ImTextCountUtf8BytesFromStr(state->TextW.Data + ib, state->TextW.Data + ie) + 1;
                        ...
                        io.ContextChoice = true;
                    }

                    if (Selectable("Cut", false))
                    {
                        if (!state->HasSelection())
                            state->SelectAll();
                        state->CursorFollow = true;
            ...
            {
                ImGui::Begin("##context-menu2", 0, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing);
                {
                    if (Selectable("Copy", false))
                    {
                        const int ib = state->HasSelection() ? ImMin(state->Stb.select_start, state->Stb.select_end) : 0;
            ...
            if (io.ContextChoice && !render_cursor || !render_selection)
            {
                if (io.ContextSleep == 20)
                {
                    io.ContextRightClicked = false;
                    io.ContextTextHovered = false;
                    io.ContextSetWindowSize = false;
                    io.ContextChoice = false;
                    io.ContextSleep = 0;
                }
                else
                    io.ContextSleep++;
